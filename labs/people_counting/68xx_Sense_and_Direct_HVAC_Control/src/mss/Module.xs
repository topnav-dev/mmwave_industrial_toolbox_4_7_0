...
function modBuild()
{
    ...
    var incFiles = libUtility.listAllFiles (".h", "mss", false);
    for (var k = 0 ; k < incFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = incFiles[k];

    /* Add all the .c files to the release package. */
    var srcFiles = libUtility.listAllFiles (".c", "mss", false);
    ...
    var cfgFiles = libUtility.listAllFiles (".cfg", "mss", false);
    ...
    var makFiles = libUtility.listAllFiles (".mak", "mss", false);
    ...
    var cmdFiles = libUtility.listAllFiles (".cmd", "mss", false);
    ...
    var cmdFiles = libUtility.listAllFiles (".txt", "mss", false);
    for (var k = 0 ; k < cmdFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = cmdFiles[k];
