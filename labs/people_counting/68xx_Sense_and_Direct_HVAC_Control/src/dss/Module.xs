...
function modBuild()
{
    ...
    var incFiles = libUtility.listAllFiles (".h", "dss", false);
    for (var k = 0 ; k < incFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = incFiles[k];

    /* Add all the .c files to the release package. */
    var srcFiles = libUtility.listAllFiles (".c", "dss", false);
    ...
    var cfgFiles = libUtility.listAllFiles (".cfg", "dss", false);
    ...
    var makFiles = libUtility.listAllFiles (".mak", "dss", false);
    ...
    var cmdFiles = libUtility.listAllFiles (".cmd", "dss", false);
    ...
    var cmdFiles = libUtility.listAllFiles (".txt", "dss", false);
    for (var k = 0 ; k < cmdFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = cmdFiles[k];
